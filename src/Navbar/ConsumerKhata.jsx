...
const ConsumerKhata = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://maherdairy.com/consumerkhata');
      if (!response.ok) {
      ...
      const updatedConsumersData = await Promise.all(consumersData.map(async (consumer) => {
        const wasooliResponse = await fetch(`https://maherdairy.com/wasooli/${consumer.idconsumerkhata}`);
  ...
  const handleSaveClick = async () => {
    ...
    const endpoint = idconsumerkhata ? `https://maherdairy.com/consumerkhata/${idconsumerkhata}` : 'https://maherdairy.com/consumerkhata';
  ...
  const handleSaveBaqayaClick = async () => {
    ...
    try {
      const response = await fetch(`https://maherdairy.com/consumerkhata/${selectedConsumerId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...consumerToUpdate,
  ...
  const handleConfirmDeletion = async () => {
    ...
    try {
      ...
      const deleteResponse = await fetch(`https://maherdairy.com/wasooli/${wasooliId}`, { method: 'DELETE' });
  ...
  const handleSaveWasooliClick = async () => {
    try {
      ...
      let endpoint = 'https://maherdairy.com/wasooli';
      let method = 'POST';
