...
const Employee = () => {
  ...
  const [transactionsVisibility, setTransactionsVisibility] = useState({});
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('https://maherdairy.com/employeekhata');
      if (!response.ok) {
      ...
      const updatedConsumersData = await Promise.all(consumersData.map(async (consumer) => {
        const wasooliResponse = await fetch(`https://maherdairy.com/kharchay/${consumer.idEmployeekhata}`);
  ...
  const handleSaveClick = async () => {
    ...
    let errors = {};
    console.log('Sending date to server:', consumerData.date);
    ...
    const endpoint = idEmployeekhata ? `https://maherdairy.com/employeekhata/${idEmployeekhata}` : 'https://maherdairy.com/employeekhata';
  ...
  const handleSaveBaqayaClick = async () => {
    ...
    try {
      const response = await fetch(`https://maherdairy.com/employeekhata/${selectedConsumerId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleConfirmDeletion = async () => {
    ...
    try {
      ...
      const deleteResponse = await fetch(`https://maherdairy.com/kharchay/${wasooliId}`, { method: 'DELETE' });
  ...
  const handleSaveWasooliClick = async () => {
    try {
      ...
      let endpoint = 'https://maherdairy.com/kharchay';
      let method = 'POST';
